...
def build_save_dataset(corpus_type, fields, src_reader, tgt_reader,
    ...
    srcs = opt.data[corpus_type]['path_src']
    tgts = opt.data[corpus_type]['path_tgt']
...
def preprocess(opt):
    ...
    tgt_nfeats = 0
    print(opt.data)
    opt.data = json.loads(opt.data.replace('\'','\"'))
    for src, tgt in zip([opt.data['train']['path_src']],[opt.data['train']['path_tgt']]):
...
def _get_parser():
    ...
    opts._add_dynamic_fields_opts(parser,False)
    opts._add_dynamic_corpus_opts(parser,True)
    opts._add_reproducibility_opts(parser)
    opts._add_logging_opts(parser)
    return parser
